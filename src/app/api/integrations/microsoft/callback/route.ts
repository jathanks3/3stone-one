...
import { consumeMicrosoftAuthState, completeMicrosoftConnection } from "@/server/services/microsoftIntegrationService";
...
export async function GET(req: Request) {
  ...
  try {
    const { workspaceId, userId } = await consumeMicrosoftAuthState(state);
    ...
    await completeMicrosoftConnection(workspaceId, userId, code, redirectUri);
  } catch (e) {
    console.error("[api/integrations/microsoft/callback] failed:", e);
