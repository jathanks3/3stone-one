...
import { buildSalesforceAuthUrl, createSalesforceAuthState, isSalesforceIntegrationConfigured } from "@/server/services/salesforceIntegrationService";
...
export async function GET(req: Request) {
  ...
  if (!isSalesforceIntegrationConfigured()) return NextResponse.redirect(new URL("/integrations?error=not_configured", req.url));
  const state = await createSalesforceAuthState(membership.workspaceId, session.userId);
  const redirectUri = `${new URL(req.url).origin}/api/integrations/salesforce/callback`;
  return NextResponse.redirect(buildSalesforceAuthUrl(state, redirectUri));
}
